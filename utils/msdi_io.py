...
def load_img(entry, msdi_path=msdi_path):
    return plt.imread(Path(msdi_path) / entry['img'])

def load_deep_audio_features(entry, msdi_path=msdi_path):
...
def get_label_list(msdi_path=msdi_path):
...
if __name__ == '__main__':
    ...
    print(one_entry)
    print('#' * 80)
    mfcc = load_mfcc(one_entry, msdi_path)
    print('MFCC shape:', mfcc.shape)
    img = load_img(one_entry, msdi_path)
    print('Image shape:', img.shape)
    deep_features = load_deep_audio_features(one_entry, msdi_path)
